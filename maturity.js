...
function showResults() {
  ...
  // Keep this as a single sentence (so existing styling stays sane)
  document.getElementById('resultStageDesc').textContent = stage.core || '';

  // New: render the “Characteristics / Risk / Mindset” block separately
  ...
  if (detailsEl) {
    ...
    detailsEl.innerHTML = `
      <div class="stage-block">
        ...
        <ul class="stage-block-list">${list(stage.characteristics)}</ul>

        <div class="stage-block-title">Risk profile</div>
        <ul class="stage-block-list">${list(stage.risks)}</ul>

        <div class="stage-block-title">Mindset</div>
        <p class="stage-mindset">${stage.mindset || ''}</p>
      </div>
    `;
